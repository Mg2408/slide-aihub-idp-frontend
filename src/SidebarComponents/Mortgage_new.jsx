...
// Policy sub-fields in display order
const POLICY_FIELDS = [
...
// Address object sub-fields in display order
...
/* ─────────────────────────────────────────
   ...
   Columns:
     ...
     Mortgagee Street | Mortgagee City | Mortgagee State | Mortgagee ZIP |
     Policy Number | Loan Number | Borrowers | Borrowers Address | Reference
───────────────────────────────────────── */
const buildExpandedExcelData = (json, documentName) => {
  ...
  const baseRow = {
    ...
    "Mortgagee Street":          getFieldValue(addrObj.Street),
  ...
  const rows =
    policies.length > 0
      ? policies.map((p) => {
          ...
          const page = getPageNumber(obj.Policy_Number?.source ?? "");
          return {
            ...baseRow,
            "Policy Number":    getFieldValue(obj.Policy_Number),
            "Loan Number":      getFieldValue(obj.Loan_Number),
            "Borrowers":        getFieldValue(obj.Borrowers),
      ...
      : [
          {
            ...baseRow,
            "Policy Number": "", "Loan Number": "",
            "Borrowers": "", "Borrowers Address": "", "Reference": "",
          },
        ];
...
const FieldRow = ({ label, field, style = {} }) => {
  ...
  const page      = getPageNumber(field.source);
  const confColor = conf >= 90 ? "green" : conf >= 70 ? "orange" : "red";
...
const Mortgage = () => {
    ...
    const fetchDocuments = async () => {
      try {
        ...
        const response = await fetch(
          buildAiExtractUrl("/api/get_extracted_documents?template=mortgage")
        );
  ...
  const renderFieldGroup = (group) => {
    ...
    /* ── Address of Mortgagee Company (valueObject with sub-fields) ── */
            ...
            {group.entries.map((entry, index) => {
              ...
              return (
                ...
                >
                  ...
                  <Col span={10} style={{ textAlign: "right" }}>
                    {confidence && (
                      <Tag color={confidence.color}>Confidence: {confidence.value}</Tag>
                    )}
                    {reference && (
                      <Tag>
                        {(() => {
        ...
        <Upload.Dragger
          ...
          customRequest={async ({ file, onSuccess, onError }) => {
            try {
              ...
              const response = await fetch(
                buildAiExtractUrl("/api/extract_document?template=mortgage"),
